...
class SleepTracker(Star):
    ...
    MORNING_DEFAULT_PROMPT = "用温暖亲切的语气祝用户早安，先问候再告知睡眠数据（入睡时间、醒来时间、睡眠时长），根据睡眠时长给出具体建议，确保符合人设并考虑上下文，确保对话通顺不突兀"
    NIGHT_DEFAULT_PROMPT = "用温柔的语气祝用户晚安，根据当前时间给出睡眠建议，确保符合人设并考虑上下文，确保对话通顺不突兀"
    STATS_DEFAULT_PROMPT = "先清晰汇报指定日期的睡眠数据（日期、入睡时间、醒来时间、睡眠时长），然后分析睡眠质量（优秀/良好/需改进），给出具体的改善建议，最后表达关心，确保符合人设并考虑上下文，确保对话通顺不突兀"
    STATIS_DEFAULT_PROMPT = "汇报时间段内的睡眠统计数据（平均睡眠时长、最早/最晚入睡时间、睡眠规律性），分析睡眠趋势和规律，指出需要改进的地方，给出周期性的健康建议，确保符合人设并考虑上下文，确保对话通顺不突兀"
    ...
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        if config is None:
            ...
            self.morning_sup_prompt = self.MORNING_DEFAULT_PROMPT
            self.night_sup_prompt = self.NIGHT_DEFAULT_PROMPT
            self.stats_sup_prompt = self.STATS_DEFAULT_PROMPT
            self.statis_sup_prompt = self.STATIS_DEFAULT_PROMPT
        else:
            logger.debug("SleepTracker: 使用用户配置文件")
            self.morning_sup_prompt = config.get(
                "morning_sup_prompt", self.MORNING_DEFAULT_PROMPT
            )
            self.night_sup_prompt = config.get(
                "night_sup_prompt", self.NIGHT_DEFAULT_PROMPT
            )
            self.stats_sup_prompt = config.get(
                "stats_sup_prompt", self.STATS_DEFAULT_PROMPT
            )
            self.statis_sup_prompt = config.get(
                "statis_sup_prompt", self.STATIS_DEFAULT_PROMPT
            )
    ...
    async def good_night(self, event: AstrMessageEvent):
        ...
        sleep_info = await self.db_service.query_user_sleep_records(
            user_id, statis_date
        )
        if line > 0 and sleep_info:
    ...
    async def sleep_stats(
        self, event: AstrMessageEvent, statis_date: str = None, user_id: str = None
    ):
        """
    ...
    @llm_tool(name="modify_sleep_time_data")
    async def modify_sleep_time_data(
        self,
        ...
        results = []
        if sleep_str:
            line = await self.db_service.update_custom_sleep_time(
                user_id, statis_date, sleep_str
            )
            if line > 0:
                results.append(f"已将{user_id}的入睡时间修改为{sleep_str}")
        if wake_str:
            line = await self.db_service.update_custom_wake_time(
                user_id, statis_date, wake_str
            )
            if line > 0:
    ...
    async def statis_sleep_data(
        ...
        # 获取用户
        user_id = event.get_sender_id()
        sleep_data = await self.db_service.statis_sleep_data(
        ...
        if sleep_data:
            for sleep_record in sleep_data:
                result_lines.append(
                    f"日期: {sleep_record['status_date']}, 入睡: {sleep_record['sleep_time']}, 醒来: {sleep_record['wake_time']}, 时长: {sleep_record['sleep_duration_minutes']}"
                )
            # 将所有行拼接成一个完整的字符串
            result = "\n".join(result_lines)
